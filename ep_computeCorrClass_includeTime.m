%ep_computeCorrClass_includeTime
%For each subject, for each ROI, for each condition, for each rep:
%1) Create the training pattern (VxT matrix, avg'd across N-1 reps).
...
for s = 1:length(subjects)
    ...
    for ROI = 1:nROIs
        ...
        for i = 1:n_scramble_reps
            ...
            %Create 1B training pattern (VxT)
            train_1B = mean(data_scramble_thisROI(:,:,1,train_reps),4);
            
            %Create 2B training pattern (VxT)
            train_2B = mean(data_scramble_thisROI(:,:,2,train_reps),4);
            
            %Create 8B training pattern (VxT)
            train_8B = mean(data_scramble_thisROI(:,:,3,train_reps),4);
            
            %Create I training pattern (VxT)
            train_I = mean(data_scramble_thisROI(:,:,4,train_reps),4);
            
            %Create 1B test (held-out) pattern (VxT)
            test_1B = data_scramble_thisROI(:,:,1,test_rep);
            
            %Create 2B test (held-out) pattern (VxT)
            test_2B = data_scramble_thisROI(:,:,2,test_rep);
            
            %Create 8B test (held-out) pattern (VxT)
            test_8B = data_scramble_thisROI(:,:,3,test_rep);
            
            %Create I test (held-out) pattern (VxT)
...
print(gcf, '-dtiff', ['../figures/Correlation classifier/Corr classifier_includeTime_' group 'group_' data_type(1:2) '_n_cropped_TRs=' num2str(n_cropped_TRs)]);
...
print(gcf, '-dtiff', ['../figures/Correlation classifier/Corr classifier_includeTime_' group 'group_' data_type(1:2) '_n_cropped_TRs=' num2str(n_cropped_TRs) '_above_chance.tif']);
