...
for s = 1:nSubs
    ...
    ROI_acc_sections = zeros(nROIs,n_sections);
    ...
    for ROI = 1:nROIs
        ...
        ROI_acc_sections(ROI,1) = mean(acc_1);
        ROI_acc_sections(ROI,2) = mean(acc_2);
        ROI_acc_sections(ROI,3) = mean(acc_3);
        ROI_acc_sections(ROI,4) = mean(acc_4);
    end
    
    %Plot classification accuracy (ROI x cond) for this subject (all 4 sections)
    figsize = [100 100 400 500]; figure('Units', 'pixels', 'Position', figsize); 
    imagesc(ROI_acc_sections); xlabel('Section'); ylabel('ROI'); set(gca, 'YTickLabel', ROIs, 'FontSize', 16, 'FontName', 'Helvetica'); colorbar; caxis([0 1]);
